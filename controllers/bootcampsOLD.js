// originally used here in the catch block, but moved to the errorHandler
...
exports.getBootcamp = async (req, res, next) => {
    try {
        ...
        // next(error);
        // instead of just passing in Error & having our custom error handler run,
        // we want to use the Custom Error Object we created in `utils/errorResponse`
        // next(new ErrorResponse(`Bootcamp not found with ID of ${req.params.id}`, 404));

        // now we want to have this logic ^ handled from within the errorHandler instead of putting it in the `catch
        next(error);
    }
